...
class Model(nn.Module):
    ...
    def rep_to_logits_mapping(self):
        embedding_weight = self.model.get_output_embeddings().weight
    ...
    def get_probabilities(self,
        ...
        with torch.set_grad_enabled(not no_grads):
            ...
            labels_strings=self.tokenizer.tokenize(text)
            ...
            for i in range(0, labels.size(1), self.stride):
                ...
                ## Get representations of all layers
                my_output=self.model(input_ids, labels=target_ids, output_hidden_states = True)
                embedding_weight = self.model.get_output_embeddings().weight
                

                my_output["inter_logits"]=[]
                my_output["inter_token_ids"]=[]

                my_output["inter_token_ids"].append(labels_processed)
                for layer in range(len(my_output["hidden_states"])):
                    hidden_state=my_output["hidden_states"][layer]
                    inter_logits=torch.einsum('bsh, vh -> bsv', hidden_state, embedding_weight)

                    if no_grads:
                        inter_logits = inter_logits.cpu()
                    shift_inter_logits = inter_logits[..., :-1, :].contiguous()
                    
                    log_probabilities_inter = torch.nn.functional.log_softmax(shift_inter_logits, dim=-1)

                    token_ids_inter=np.argmax(log_probabilities_inter,axis=2)    

                    my_output["inter_token_ids"].append(token_ids_inter[0,:])
                    
                    # shift_labels = target_ids[..., 1:]
                    # add a new dimension to labels_processed
                    ...
                    target_token_log_prob_inter=[]
                    all_token_log_prob_inter=[]
                    for i, token_id in enumerate(labels_processed):
                        if token_id != -100:
                            log_probability_inter = log_probabilities_inter[0, i, token_id]
                            if no_grads:
                                log_probability_inter = log_probability_inter.item()

                            target_token_log_prob_inter.append(log_probability_inter)
                            all_token_log_prob_inter.append(log_probabilities_inter[0, i])
                        # all_token_log_prob_inter = torch.stack(all_token_log_prob_inter, dim=0)
                        # target_token_log_prob_inter = torch.stack(target_token_log_prob_inter)        
                    
                    all_token_log_prob_inter = torch.stack(all_token_log_prob_inter, dim=0)
                    
                    my_output["inter_logits"].append(target_token_log_prob_inter)    


                
                my_output["inter_token_ids"] = torch.stack(my_output["inter_token_ids"], dim=0)
        ...
        return target_token_log_prob, all_token_log_prob, labels_strings, my_output["inter_logits"], my_output["inter_token_ids"]  
    ...
    def get_ll_agg(self, text: str,
               tokens: np.ndarray=None,
               probs = None, agg=None, lower_bound=-1, starting_point=0, end_point=None):
        
        all_prob = probs if probs is not None else self.get_probabilities(text, tokens=tokens)
        if agg is None:
            return -np.mean(all_prob)
        elif agg == "median":
            return -np.median(all_prob)
        elif agg == "tf":
            def get_best_signal(x, min=-1, max=1):
                return -np.sum((np.array(x)[:200] >= min) & (np.array(x)[:200] < max))
            return get_best_signal(all_prob)
        elif agg == "count_above":
            if end_point is None:
                return -np.sum(np.array(all_prob)[starting_point:] >= lower_bound)
            else:
                return -np.sum(np.array(all_prob)[starting_point:end_point] >= lower_bound)
